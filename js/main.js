$(document).ready(function(){
    ...
    $(close).on('click', function(){
        ...
        parent.removeClass(menu_active);
        $('.toogle_popup').removeClass('toogle_popup_open')
        $('body').removeAttr('style');
    });

    $('.toogle_popup_open').click(function(){
        $('.toogle_popup').addClass('toogle_popup_open');
        $('body').css({'overflow': 'hidden', 'margin-right': '17px'});
        $(document).mouseup(function (e) {
            if (!$(e.target).closest('.baron__scroller').length) {
                $('body').removeAttr('style');
                $('.toogle_popup').removeClass('toogle_popup_open');
            }
        });
    ...
    objectFitVideos(document.getElementsByClassName('cover'));
});
